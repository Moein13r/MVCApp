...
{        
    public string ConnectionString { get; set; }
        public DataTable GetQueryResult(string queryText)
        {
            ...
            DataSet dset = new(tableName);
            using (SqlConnection sqlConnection = new(ConnectionString))
            {
                using (SqlDataAdapter dAdapter = new(queryText, sqlConnection))
                {
                    try
                    {
                        sqlConnection.Open();
        ...
        public object GetQuerySingleResult(string queryText, List<SqlParameter> parameters)
        {
            string tableName = "QueryResultTable";
            DataSet dset = new DataSet(tableName);
            using (SqlConnection sqlConnection = new SqlConnection(ConnectionString))
            {
                using (SqlDataAdapter dAdapter = new SqlDataAdapter(queryText, sqlConnection))
                {
                    try
                    {
                        ...
                        dAdapter.SelectCommand.Parameters.AddRange(parameters.ToArray());
                        dAdapter.Fill(dset, tableName);
                    }
                    catch (Exception error)
                    {
        ...
        public DataTable GetQueryResult(string storeProcedureName, List<SqlParameter> parameters)
        {
            ...
            return dset.Tables[tableName].Rows[0].Table;
        }
        ...
        public object GetSingleObjectResult(string queryText,List<SqlParameter> parameters)
        {
            object result = null;
            using (SqlConnection sqlConnection = new SqlConnection(ConnectionString))
            {
        ...
        public int GetNonQueryExecutionResult(string queryText, List<SqlParameter> parameters)
        {
            int result = 0;
            using (SqlConnection sqlConnection = new SqlConnection(ConnectionString))
            {
